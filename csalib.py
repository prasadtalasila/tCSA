...
def printConnection(x,SA,Days):
    ...
    #if hr1 > hr2 : Days += 1
    print "{} {} to {} {}:{} {}:{} {} {}".format(x.train_no,SA[x.dept_stn],SA[x.arr_stn],hr1,min1,hr2,min2,x.label,Days)
    return



#plain connection scan algorithm (CSA); algorithm-1 of the paper
def CSA(Departure_Station, Arrival_Station, Departure_Time,SA,CA,Days_Of_Travel):
    ...
    No_of_Days = 0
    i = 0
    while No_of_Days < Days_Of_Travel:
    #for i in range(len(CA)):
        Ci = CA[i]
        Ci.dept_time += No_of_Days*24*60*60
        Ci.arr_time += No_of_Days*24*60*60
...
def tCSA(Departure_Station, Arrival_Station, Departure_Time,DCTable,SA,TT,OMTable,CA,Days_Of_Travel):
...
def Itinerary(IC,Departure_Station,Arrival_Station,SA):
    if IC[Arrival_Station] == None :
        ...
        prev = Route[0]
        for x in Route:
            if x.dept_time%86400 > x.arr_time%86400 : Days += 1
            elif x.dept_time%86400 < prev.arr_time%86400 : Days +=1
            prev = x
            printConnection(x,SA,Days)
